...
while command:
    ...
    if is_in_matrix_boundaries(next_row, next_col, matrix):
        ...
        if matrix[santa_row][santa_col] == "V":
            ...
            for move, step in direction.items():  # check all directions for a children
                next_row_check, next_col_check = step(santa_row, santa_col)
                if is_in_matrix_boundaries(next_row_check, next_col_check,
                                           matrix):  # in case of "C" is on border row/col cell, shouldnt be by def.
                    ...
                    if not matrix[next_row_check][next_col_check] == "C":
                        matrix[next_row_check][
                            next_col_check] = "-"  # replaces all children found who received presents with "-"
